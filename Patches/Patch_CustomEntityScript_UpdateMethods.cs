...
{
    [HarmonyPatch]
    internal static class Patch_CustomEntityScript_UpdateMethods
    {
        [HarmonyTargetMethods]
        public static MethodBase[] TargetMethods()
        {
            MethodBase[] methods = GadgetMods.ListAllMods().SelectMany(x => x.Assembly.GetExportedTypes()).Where(x => typeof(CustomEntityScript).IsAssignableFrom(x)).SelectMany(x => new[] { x.GetMethod("Update", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance), x.GetMethod("FixedUpdate", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance) }).Where(x => x != null).ToArray();
